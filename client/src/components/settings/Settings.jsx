...
function Settings() {
  ...
  const [addCollabs, setAddCollabs] = useState({ fields });
  const [isPopupVisible, setIsPopupVisible] = useState(false);
  const formRef = useRef(null); // Reference for scrolling
  // to fetch all collaborators
  const getCollabs = async (req, res) => {
    try {
      const response = await axios.get(
        `http://localhost:9000/api/v1/admin/getallcollaborators`
      );
  ...
  const handleChange = (e) => {
    ...
    console.log("onchange", addCollabs);
  };
  const addCollaborators = async (req, res) => {
    try {
      const response = await axios.post(
        `http://localhost:9000/api/v1/collaborator/registercollaborator`,
        addCollabs
      );
            ...
            <div className="settingsModalDiv-content">
              <div className="outer-box" style={{ position: "relative" }}>
                ...
                <div className="form-container">
                  ...
                  <button className="add-email-button1">
                    + Add Email Address
                  </button>
                </div>
              </div>
